...
namespace Naninovel
{
    ...
    public class MetadataMapEditor
    {
        ...
        private void DrawListElement (Rect rect, int index, bool selected, bool focused)
        {
            ...
            {
                if (IsLocked(initialIdValue)) return;
                HandleListElementRemoved(reorderableList);
                Event.current.Use();
                return;
            }

            EditorGUI.BeginDisabledGroup(IsLocked(initialIdValue));
            ...
            if (GUI.Button(propertyRect, GUIContents.EditIcon, EditorStyles.miniButton))
                EditMetaProperty(metaProperty);
            EditorGUI.EndDisabledGroup();
        }
        ...
        private bool IsLocked (string actorId)
        {
            return lockedIds != null && lockedIds.Contains(actorId);
        }
    }
}
