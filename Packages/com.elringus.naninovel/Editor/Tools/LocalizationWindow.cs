...
namespace Naninovel
{
    public class LocalizationWindow : EditorWindow
    {
        ...
        [MenuItem(MenuPath.Root + "/Tools/Localization")]
        ...
        private void ValidatePaths ()
        {
            ...
            if (LocaleFolderPath != null && Directory.Exists(LocaleFolderPath) && LocaleFolderPath.EndsWithOrdinal(localizationRoot))
        ...
        private void OnGUI ()
        {
            using var _ = EditorUtils.ScrollView(nameof(LocalizationWindow));

            EditorGUILayout.LabelField("Naninovel Localization", EditorStyles.boldLabel);
            EditorGUILayout.LabelField("The tool to generate localization documents; see Localization guide for usage instructions.", EditorStyles.wordWrappedMiniLabel);
        ...
        private void LocalizeScripts (string localeFolderPath)
        {
            ...
            string ResolveLocalizationDocumentFilePath (string sourceFilePath, string sourceDirPath)
            {
                ...
                if (!sourceDirPath.EndsWithOrdinal("/")) sourceDirPath += "/";
            ...
            void WarnUntranslated (TextAsset doc)
            {
                ...
                foreach (var key in missingKeys)
                    for (int i = 0; i < lines.Length; i++)
                    {
                        ...
                        Engine.Warn($"{EditorUtils.BuildAssetLink(doc, i)} localization document is missing '{key}' key translation at line #{i + 1}.");
                        break;
                    }
                missingKeys.Clear();
            }
        ...
        private string PrependHeader (string documentText, string localeFolderPath, string sourceName)
        {
            ...
            {
                if (!documentText.StartsWithOrdinal("\n")) header += '\n';
                if (!documentText.StartsWithOrdinal("\n\n")) header += '\n';
            }
