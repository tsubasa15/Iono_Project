...
namespace Naninovel
{
    public class VoiceMapWindow : EditorWindow
    {
        ...
        [MenuItem(MenuPath.Root + "/Tools/Voice Map")]
        ...
        private void OnGUI ()
        {
            using var _ = EditorUtils.ScrollView(nameof(VoiceMapWindow));
        ...
        private void UpdateSelectedScript (bool saveBeforeUpdate = true)
        {
            if (saveBeforeUpdate) SaveResources();
            map.Clear();
            if (!script) return;
        ...
        private void ProcessCommand (Commands.PrintText cmd)
        {
            ...
            var label = $"#{cmd.PlaybackSpot.LineNumber}{(cmd.PlaybackSpot.InlineIndex == 0 ? "" : $".{cmd.PlaybackSpot.InlineIndex}")} ";
            if (!string.IsNullOrEmpty(cmd.AuthorId)) label += $"{cmd.AuthorId}: ";
            label += ResolveText(cmd.Text);
            map[fullPath] = new() {
                Label = new(label, label),
                ClipGuid = Assets.Get(fullPath)?.Guid,
        ...
        private void SaveResources ()
        {
            ...
            var prefix = string.IsNullOrEmpty(locale)
            ...
            {
                var fullPath = kv.Key;
                var path = fullPath.GetAfterFirst(prefix + "/");
                var item = kv.Value;
                Assets.Unregister(fullPath);
                if (item.Assigned)
                {
                    Assets.Register(new(item.ClipGuid, prefix, path));
                    Addressables.Register(item.ClipGuid, prefix, path);
                }
            }
        }
