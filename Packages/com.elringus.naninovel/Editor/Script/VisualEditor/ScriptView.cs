...
namespace Naninovel
{
    public class ScriptView : VisualElement
    {
        ...
        public void GenerateForScript (string scriptText, Script scriptAsset, bool forceRebuild = false)
        {
            ...
            if (!forceRebuild && lastGeneratedTextHash == scriptText.GetHashCode())
            {
                ...
                if (Engine.Initialized && Engine.Behaviour is RuntimeBehaviour)
                    HighlightPlayedCommand(Engine.GetService<IScriptPlayer>()?.MainTrack);
                return;
            }
        ...
        public ScriptLineView CreateLineView (int lineIndex, string lineText)
        {
            ...
            if (line is Syntax.CommentLine comment) return new CommentLineView(lineIndex, comment, linesContainer);
            if (line is Syntax.LabelLine label) return new LabelLineView(lineIndex, label, linesContainer);
            if (line is Syntax.CommandLine command) return CommandLineView.CreateOrError(lineIndex, lineText, command, errors, linesContainer, config.HideUnusedParameters);
            var indent = ((Syntax.GenericLine)Compiler.ParseLine(lineText)).Indent;
        ...
        private void ContextMenu (ContextualMenuPopulateEvent evt)
        {
            ...
            var hoveringLine = nearLine != null && nearLine.ContainsPoint(new(nearLine.resolvedStyle.translate.x, nearLine.WorldToLocal(evt.mousePosition).y));
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode && hoveringLine && (nearLine is CommandLineView || nearLine is GenericTextLineView))
            {
                ...
                if (state != null && track != null && track.PlayedScript && track.PlayedScript.Path == scriptAsset.Path)
                {
                    var rewindIndex = ViewToGlobalIndex(nearLineViewIndex);
                    var status = (rewindIndex > track.PlaybackSpot.LineIndex ||
                                  state.CanRollbackTo(s => s.PlaybackSpot.ScriptPath == track.PlayedScript.Path && s.PlaybackSpot.LineIndex == rewindIndex))
                        ? DropdownMenuAction.Status.Normal
                        : DropdownMenuAction.Status.Disabled;
                    evt.menu.AppendAction("Rewind", _ => track.Rewind(rewindIndex).Forget(), status);
                }
            }
            if (hoveringLine)
            {
                if (nearLine is CommandLineView cmdLine && (cmdLine.CommandId.EqualsIgnoreCase("goto") || cmdLine.CommandId.EqualsIgnoreCase("gosub")))
                {
                    var path = cmdLine.Q<LineTextField>().value;
                    evt.menu.AppendAction($"Open `{path}`", _ => HandleGoto(path));
                }
                else if (nearLine.Q("Content")?.Children().FirstOrDefault(c => c is LineTextField field && field.label.EqualsIgnoreCase("goto")) is LineTextField gotoField)
            ...
            void HandleGoto (string path)
            {
                ...
                {
                    var fullPath = string.IsNullOrEmpty(config.Loader.PathPrefix) ? scriptPath : $"{config.Loader.PathPrefix}/{scriptPath}";
                ...
                else if (!string.IsNullOrEmpty(scriptLabel))
                {
                    if (linesContainer.Children().FirstOrDefault(l =>
                            ...
                            labelLine.ValueField.value.EqualsOrdinal(scriptLabel)) is LabelLineView line)
                    {
                        FocusLine(line);
                        ScrollToLine(line);
                    }
        ...
        private void OpenHelpFor (ScriptLineView line)
        {
            ...
            url += line switch {
                CommentLineView => "guide/scenario-scripting#comment-lines",
                LabelLineView => "guide/scenario-scripting#label-lines",
                GenericTextLineView => "guide/scenario-scripting#generic-text-lines",
                CommandLineView commandLine => $"api/#{commandLine.CommandId.ToLowerInvariant()}",
                ErrorLineView errorLine => $"api/#{errorLine.CommandId}",
                _ => "guide/scenario-scripting"
            };
        ...
        private void ShowSearcher (Vector2 position, int insertIndex, int insertViewIndex)
        {
            SearcherWindow.Show(EditorWindow.focusedWindow, searchItems, "Insert Line", item => {
                ...
                switch (item.Name)
                {
                    ...
                    case "Comment":
                        lineText = Compiler.Symbols.CommentLine;
                        break;
                    case "Label":
                        lineText = Compiler.Symbols.LabelLine;
                        break;
                    case "Generic Text":
        ...
        private void HandleMouseDownEvent (MouseDownEvent evt)
        {
            ...
            var track = Engine.GetService<IScriptPlayer>()?.MainTrack;
            var state = Engine.GetService<IStateManager>();
            if (state is null || track is null || !track.PlayedScript || track.PlayedScript.Path != scriptAsset.Path) return;

            var rewindIndex = ViewToGlobalIndex(nearLineViewIndex);
            if (rewindIndex == track.PlaybackSpot.LineIndex) return;
            if (rewindIndex < track.PlaybackSpot.LineIndex)
            {
                var lastIndex = track.Playlist.GetLastCommand()?.PlaybackSpot.LineIndex ?? -1;
                bool CanRollback () => state.CanRollbackTo(s => s.PlaybackSpot.ScriptPath == track.PlaybackSpot.ScriptPath && s.PlaybackSpot.LineIndex == rewindIndex);
            ...
            track.Rewind(rewindIndex).Forget();
        }
        ...
        private void HandleEngineInitialized ()
        {
            ...
            player.OnExecute += HighlightPlayedCommand;
            player.OnAwaitInput += HandleAwaitInput;
            var stateManager = Engine.GetServiceOrErr<IStateManager>();
            stateManager.OnRollbackFinished += () => HighlightPlayedCommand(player.MainTrack);
            if (player.MainTrack.PlayedCommand != null)
                HighlightPlayedCommand(player.MainTrack);
        }

        private void HighlightPlayedCommand (IScriptTrack track)
        {
            if (Selection.activeObject is not Script ||
                Engine.GetService<IScriptPlayer>() is not { } player ||
                player.MainTrack != track ||
                track.PlayedCommand is not { } command) return;
            ...
            if (!scriptAsset || scriptPath != scriptAsset.Path)
            {
                ...
                var path = PathUtils.Combine(PackagePath.ScenarioRoot, scriptPath + ".nani");
            ...
            playedLine.AddToClassList(track.AwaitingInput ? waitInputLineClass : playedLineClass);

            ScrollToLine(playedLine);
        }

        private void HandleAwaitInput (IScriptTrack track)
        {
            if (Engine.GetService<IScriptPlayer>() is not { } player ||
                player.MainTrack != track ||
                !track.AwaitingInput) return;

            if (track.PlayedCommand is null || !scriptAsset ||
                track.PlaybackSpot.ScriptPath != scriptAsset.Path ||
                !Lines.IsIndexValid(track.PlaybackSpot.LineIndex) ||
                !ViewRange.Contains(track.PlaybackSpot.LineIndex))
                return;

            var playedLine = Lines[track.PlaybackSpot.LineIndex];
