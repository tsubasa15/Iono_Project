...
namespace Naninovel
{
    public class CommandLineView : ScriptLineView
    {
        ...
        {
            var lineText = $"{Compiler.Symbols.CommandLine}{commandId}";
            var line = (Syntax.CommandLine)Compiler.ParseLine(lineText);
            return CreateOrError(lineIndex, lineText, line, null, container, hideParameters);
        }

        public static ScriptLineView CreateOrError (int lineIndex, string lineText, Syntax.CommandLine line,
            ErrorCollector errors, VisualElement container, bool hideParameters)
        {
            ...
            var cmdMeta = metadata.FirstOrDefault(c =>
                c.Id.EqualsIgnoreCase(cmdId) ||
                (c.Alias?.EqualsIgnoreCase(cmdId) ?? false));
            ...
            string GetValueFor (Metadata.Parameter m)
            {
                var param = cmdModel.Parameters.FirstOrDefault(p => p.Nameless && m.Nameless || p.Identifier != null &&
                    (p.Identifier.Text.EqualsIgnoreCase(m.Id) || p.Identifier.Text.EqualsIgnoreCase(m.Alias)));
                if (param is null) return null;
                return Compiler.SyntaxFormatter.Format(param.Value, new() { ParameterValue = true, NamelessParameterValue = param.Nameless });
            }
        }

        public override string GenerateLineText ()
        {
            var result = $"{GenerateLineIndent()}{Compiler.Symbols.CommandLine}{CommandId}";
            ...
            foreach (var data in parameterFields)
                if (!string.IsNullOrWhiteSpace(data.Field.value))
                    ...
                    else if (data.Field.value?.EqualsIgnoreCase("true") ?? false) parameters.Add($"{data.Id}!");
                    else if (data.Field.value?.EqualsIgnoreCase("false") ?? false) parameters.Add($"!{data.Id}");
