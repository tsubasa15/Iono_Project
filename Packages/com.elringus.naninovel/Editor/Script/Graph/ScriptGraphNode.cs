    ...
    public sealed class ScriptGraphNode : Node
    {
        ...
        public ScriptGraphNode (ScriptsConfiguration config, Script script, string scriptGuid, List<Script> availableScripts)
        {
            ...
            foreach (var line in script.Lines)
            {
                ...
                {
                    InputPorts[labelLine.LabelText] = AddPort(Direction.Input, $"{Compiler.Symbols.LabelLine}{labelLine.LabelText}");
                    continue;
                }
                ...
                if (line is CommandLine commandLine)
                {
                    if (commandLine.Command is Goto @goto && !(@goto.Path?.DynamicValue ?? true))
                        AddOutPort(@goto, ResolveEndpoint(@goto.Path)?.ScriptPath ?? script.name, ResolveEndpoint(@goto.Path)?.Label, $"goto {@goto.Path}");
                    if (commandLine.Command is Gosub gosub && !(gosub.Path?.DynamicValue ?? true))
                        AddOutPort(gosub, ResolveEndpoint(gosub.Path)?.ScriptPath ?? script.name, ResolveEndpoint(gosub.Path)?.Label, $"gosub {gosub.Path}");
                    if (commandLine.Command is AddChoice choice && Command.Assigned(choice.GotoPath) && !choice.GotoPath.DynamicValue)
                        AddOutPort(choice, ResolveEndpoint(choice.GotoPath)?.ScriptPath ?? script.name, ResolveEndpoint(choice.GotoPath)?.Label, $"choice goto:{choice.GotoPath}");
                }
            }

            void AddOutPort (Command command, string gotoScript, string gotoLabel, string portLabel)
            {
                portLabel = $"{Compiler.Symbols.CommandLine}{portLabel}";
                ...
                {
                    tooltip = command.InvertedConditionalExpression?.DynamicValue ?? false
                        ? command.InvertedConditionalExpression.RawValue.ToString()
                        : command.InvertedConditionalExpression?.Value;
        ...
        private static VisualElement CreateSynopsis (Script script)
        {
            ...
            {
                if (scriptLine is CommentLine commentLine)
                {
        ...
        private Endpoint? ResolveEndpoint (string syntax)
        {
            return EditorEndpointResolver.Resolve(syntax, ScriptPath);
        }
