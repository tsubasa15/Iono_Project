...
namespace Naninovel
{
    ...
    public static class ScriptFileWatcher
    {
        ...
        private static void StopWatching ()
        {
            ...
            while (modifiedScriptPaths.TryDequeue(out _)) ;
        }

        private static void Update ()
        {
            if (modifiedScriptPaths.IsEmpty || (DateTime.Now - lastEventTime).TotalMilliseconds < debounceMs) return;

            using var _ = SetPool<string>.Rent(out var paths);
            while (modifiedScriptPaths.TryDequeue(out var path))
                paths.Add(path);
            ...
            foreach (var path in paths)
            {
                ...
                var assetPath = PathUtils.AbsoluteToAssetPath(path);
            ...
            if (processedAny && !InternalEditorUtility.isApplicationActive)
        ...
        {
            var scenarioRoot = PackagePath.ScenarioRoot;
            ...
            static bool ContainsScripts (string path) => Directory.GetFiles(path, "*.nani", SearchOption.TopDirectoryOnly).Length > 0;
        }
        ...
        private static FileSystemWatcher CreateWatcher (string path)
        {
            ...
            watcher.Filter = "*.nani";
            watcher.Changed += (_, e) => {
                modifiedScriptPaths.Enqueue(e.FullPath);
        ...
        private static void DisposeWatcher (Task<FileSystemWatcher> startTask)
        {
            ...
            catch (Exception e) { Engine.Err($"Failed to stop script file watcher: {e.Message}"); }
        }
    }
}
