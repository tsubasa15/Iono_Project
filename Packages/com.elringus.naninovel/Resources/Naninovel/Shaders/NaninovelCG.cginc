...
inline float4 Clip01(float4 texColor, float2 uvCoord, float4 clipColor)
{
    const float4 UV_RANGE = float4(0, 0, 1, 1);
    float isUVOutOfRange = IsPositionOutsideRect(uvCoord, UV_RANGE);
    return lerp(texColor, clipColor, isUVOutOfRange);
}

inline float4 PremultiplyAlpha (float4 color)
{
