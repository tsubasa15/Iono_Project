Shader "Naninovel/FX/BokehURP"
{
    SubShader
    {
        ...
        float3 GammaToLinearSpace(float3 c)
        {
            return (float3)SRGBToLinear(c);
        }

        float3 LinearToGammaSpace(float3 c)
        {
            return (float3)LinearToSRGB(c);
        }

        Vertex ComputeVertex(Attributes att)
        {
            Vertex v;

            float2 pos = GetFullScreenTriangleVertexPosition(att.vertexID).xy;
            v.pos = float4(pos, 0.0, 1.0);
            float2 uv = GetFullScreenTriangleTexCoord(att.vertexID);
            v.uv0 = uv;

            float2 uv1 = uv;
            #if UNITY_UV_STARTS_AT_TOP
            if (_MainTex_TexelSize.y < 0.0) uv1.y = 1.0 - uv1.y;
            #endif
            v.uv1 = uv1;
        ...
        float4 Prefilter(Vertex v) : SV_Target
        {
            ...
            float3 c0 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 - duv.xy).rgb;
            float3 c1 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 - duv.zy).rgb;
            float3 c2 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.zy).rgb;
            float3 c3 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.xy).rgb;

            // Sample linear depths
            float d0 = OrthoEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, v.uv1 - duv.xy));
            float d1 = OrthoEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, v.uv1 - duv.zy));
            float d2 = OrthoEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, v.uv1 + duv.zy));
            float d3 = OrthoEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, v.uv1 + duv.xy));
        ...
        {
            float4 samp0 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0);
            ...
            UNITY_LOOP for (int si = 0; si < ITERATION_COUNT; si++)
            {
                ...
                float4 samp = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv);
        ...
        float4 BlurFinal(Vertex v) : SV_Target
        {
            ...
            float4 acc = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 - duv.xy);
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 - duv.wy) * 2;
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 - duv.zy);

            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.zw) * 2;
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0) * 4;
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.xw) * 2;

            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.zy);
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.wy) * 2;
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, v.uv0 + duv.xy);
        ...
        {
            float4 cs = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv0);
            float4 cb = SAMPLE_TEXTURE2D(_BlurTex, sampler_BlurTex, i.uv1);
            #if defined(UNITY_COLORSPACE_GAMMA)
            cs.rgb = GammaToLinearSpace(cs.rgb);
            #endif
            float3 rgb = cs.rgb * cb.a + cb.rgb;
            #if defined(UNITY_COLORSPACE_GAMMA)
