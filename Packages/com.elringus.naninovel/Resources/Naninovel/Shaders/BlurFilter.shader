...
Shader "Hidden/Naninovel/BlurFilter"
{
    ...
    float4 DownsampleToQuarterSize(v2f_img i) : SV_Target
    {
        const float4 deviation = _MainTex_TexelSize.xyxy * float4(1, 1, -1, -1);
        
        float4 color = tex2D(_MainTex, i.uv + deviation.xy);
    ...
    float4 Blur(const float2 uv, const float2 stride)
    {
        ...
        float4 color = tex2D(_MainTex, uv) * blurStep1;
    ...
    float4 BlurHorizontal(v2f_img i) : SV_Target
    {
        return Blur(i.uv, float2(_MainTex_TexelSize.x, 0));
    }

    float4 BlurVertical(v2f_img i) : SV_Target
    {
