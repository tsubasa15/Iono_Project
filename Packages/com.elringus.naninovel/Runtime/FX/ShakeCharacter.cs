...
namespace Naninovel.FX
{
    ...
    public class ShakeCharacter : ShakeTransform
    {
        ...
        protected override Transform GetShakenTransform ()
        {
            ...
            var id = string.IsNullOrEmpty(ObjectName) ? manager.FindActor(a => a.Visible)?.Id : ObjectName;
            if (id is null || !manager.ActorExists(id))
                throw new Error($"Failed to shake character with '{id}' ID: actor not found.");
            return (manager.GetActorOrErr(id) as MonoBehaviourActor<CharacterMetadata>)?.Transform;
        }

        protected override async Awaitable ShakeSequence (AsyncToken token)
        {
