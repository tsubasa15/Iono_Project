...
namespace Naninovel.FX
{
    public class Blur : MonoBehaviour, Spawn.IParameterized, Spawn.IAwaitable, DestroySpawned.IParameterized, DestroySpawned.IAwaitable
    {
        ...
        {
            Awaitable Blur (float intensity, Tween tween, AsyncToken token = default);
        }
        ...
        public async Awaitable AwaitSpawn (AsyncToken token = default)
        {
        ...
        public async Awaitable AwaitDestroy (AsyncToken token = default)
        {
            var actor = FindActor(ActorId);
            if (actor is null) return;
        ...
        private static IBlurable FindActor (string actorId, bool logError = true)
        {
            ...
            var blurable = manager.GetActorOrErr(actorId) as IBlurable;
            if (blurable is null)
            {
