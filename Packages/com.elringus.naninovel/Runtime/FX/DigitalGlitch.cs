...
namespace Naninovel.FX
{
    ...
    {
        protected virtual float Duration { get; private set; }
        protected virtual float Intensity { get; private set; }
        ...
        private static readonly int flipURP = Shader.PropertyToID("flipURP");
        ...
        public virtual async Awaitable AwaitSpawn (AsyncToken token = default)
        {
            ...
            await Async.Delay(System.TimeSpan.FromSeconds(Duration));
        ...
        private class CameraComponent : MonoBehaviour
        {
            ...
            {
                UnityEngine.Rendering.RenderPipelineManager.endContextRendering += HandleCameraEndRendering;
            }
            #endif
            ...
            #if URP_AVAILABLE
            private void HandleCameraEndRendering (UnityEngine.Rendering.ScriptableRenderContext context, List<Camera> camera)
            {
                UpdateMaterial();
                Material.SetFloat(flipURP, 1);
                var temp = RenderTexture.GetTemporary(Screen.width, Screen.height);
                Graphics.Blit(null, temp);
                Graphics.Blit(temp, default(RenderTexture), Material);
