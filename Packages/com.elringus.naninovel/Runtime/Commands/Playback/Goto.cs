...
..."
    ...
    [Serializable, PlaybackGroup, Icon("Right"), Branch(BranchTraits.Endpoint)]
    public class Goto : Command, Command.INavigator
    {
        /// <summary>
        ...
        bool INavigator.HoldResources => Hold;
        bool INavigator.ReleaseResources => Release;
        ...
        protected virtual IScriptLoader Loader => Engine.GetServiceOrErr<IScriptLoader>();
        ...
        {
            if (!TryGetScriptPathAndLabel(ctx.Track, out var scriptPath, out var label)) return;
            if (ShouldNavigatePlayedScript(ctx.Track, scriptPath)) NavigatePlayedScript(ctx.Track, label);
                ...
                using (await LoadingScreen.Show(ctx.Token))
                    await NavigateOtherScript(ctx.Track, scriptPath, label);
            else await NavigateOtherScript(ctx.Track, scriptPath, label);
        }
        ...
        protected virtual bool TryGetScriptPathAndLabel (IScriptTrack track, out string scriptPath, [MaybeNull] out string label)
        {
            if (!Scripts.TryResolveEndpoint(Path.Value, PlaybackSpot.ScriptPath, out var endpoint))
                Err($"Failed to execute '@goto' command: endpoint syntax is not valid: {Path.Value}");
        ...
        protected virtual async Awaitable NavigateOtherScript (IScriptTrack track, string scriptPath, [MaybeNull] string label)
        {
            ...
            Awaitable LoadAndPlay () => string.IsNullOrEmpty(label)
                ? track.LoadAndPlay(scriptPath)
                : track.LoadAndPlayAtLabel(scriptPath, label);
        }

        protected virtual bool ShouldResetAll ()
        {
            var cfg = Engine.GetConfiguration<StateConfiguration>();
            return !Assigned(ResetState) && cfg.ResetOnGoto ||
