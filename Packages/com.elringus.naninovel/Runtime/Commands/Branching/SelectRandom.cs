...
@random
    ...
    [Serializable, Alias("random"), BranchingGroup, Icon("Dice"), RequireNested, Branch(BranchTraits.Nest | BranchTraits.Return)]
    public class SelectRandom : Command, Command.INestedHost
    {
        ...
        protected virtual int PickRandomNested (ScriptPlaylist playlist, int hostIndex)
        {
            ...
            for (int i = hostIndex + 1; i < playlist.Count; i++)
            {
                ...
                {
                    var seed = UnityEngine.Random.value * (Weight?.ElementAtOrNull(++weightIndex) ?? 1f);
