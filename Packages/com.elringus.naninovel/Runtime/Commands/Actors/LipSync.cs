...
Kohaku: Lorem ipsum dolor sit amet[lipSync Kohaku.false]... [lipSync Kohaku.true]Consectetur adipiscing elit."
    ...
    [Serializable, ActorsGroup, Icon("Microphone")]
    public class LipSync : Command
    {
        ...
        [Alias(NamelessParameterAlias), RequiredParameter, ActorContext(CharactersConfiguration.DefaultPathPrefix, 0)]
        ...
        public override Awaitable Execute (ExecutionContext ctx)
        {
            ...
            if (characterManager.GetActorOrErr(CharIdAndAllow.Name) is IReceiver receiver)
                receiver.AllowLipSync(CharIdAndAllow.NamedValue ?? false);
            else Warn($"Failed to control lip sync for '{CharIdAndAllow.Name}': character is not able to receive lip sync events.");
