...
@slide Sheba from:15,50 to:85,0 time:5 easing:EaseOutBounce"
    ...
    [Serializable, Alias("slide"), ActorsGroup]
    public class SlideActor : Command
    {
        [Doc("ID of the actor to slide and (optionally) appearance to set.")]
        [Alias(NamelessParameterAlias), RequiredParameter, ActorContext(index: 0), AppearanceContext(1)]
        ...
        {
            return WaitOrForget(Slide, Wait, ctx);
        }

        protected virtual async Awaitable Slide (ExecutionContext ctx)
        {
            ...
            var fromPos = new Vector3(
                ...
                actor.Visible ? actor.Position.x : cfg.SceneToWorldSpace(new Vector2(UnityEngine.Random.value > .5f ? -.1f : 1.1f, 0)).x,
            ...
            if (!actor.Visible)
            {
                ...
                tasks.Add(actor.ChangeAppearance(IdAndAppearance.NamedValue, tween, token: ctx.Token));

            if (Assigned(Visible)) tasks.Add(actor.ChangeVisibility(Visible, tween, ctx.Token));

            tasks.Add(actor.ChangePosition(toPos, tween, ctx.Token));
