...
@char Kohaku pos:50 scale:1"
    ...
    [Serializable, Alias("animate"), ActorsGroup, Icon("WaveSine"), Ignore]
    public class AnimateActor : Command
    {
        ...
        /// </summary>
        /// <remarks>
        /// Comma should be used by default; colon is for backward compat and may be removed in the future.
        /// </remarks>
        ...
        [Doc("Type of the [transition effect](/guide/special-effects#transition-effects) to use when animating appearance change (crossfade is used by default).")]
        ...
        {
            Warn("@animate command is deprecated and will be removed in the next release — use @async instead.");

            var spawner = Engine.GetServiceOrErr<ISpawnManager>();
            using var _ = Async.Rent<SpawnedObject>(out var spawnTasks);
            foreach (var actorId in ActorIds)
                spawnTasks.Add(Spawn(spawner, actorId, ctx.Token));
            var spawned = await Async.All(spawnTasks);

            using var __ = Async.Rent(out var initTasks);
            foreach (var spawn in spawned)
                initTasks.Add(spawn.AwaitSpawn(ctx.Token));
            await WaitOrForget(ctx => Async.All(initTasks), Wait, ctx);
        }

        protected virtual async Awaitable<SpawnedObject> Spawn (ISpawnManager spawner, string actorId, AsyncToken token)
        {
            var path = BuildSpawnPath(actorId);
            var spawned = spawner.GetSpawned(path) ?? await spawner.Spawn(path, token: token);
        ...
        protected virtual IReadOnlyList<string> GetParametersForActor (string actorId)
        {
            ...
            parameters[1] = GetAssignedOrDefault(Loop, false).ToString(CultureInfo.InvariantCulture);
