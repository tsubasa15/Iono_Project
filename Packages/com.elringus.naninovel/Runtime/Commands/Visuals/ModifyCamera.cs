...
@camera set:*.false"
    ...
    [Serializable, Alias("camera"), VisualsGroup, Icon("CameraMovie")]
    public class ModifyCamera : Command
    {
        ...
        [Doc("Names of the components to toggle (enable if disabled and vice-versa). The components should be attached to the same game object as the camera. " +
             "This can be used to toggle [custom post-processing effects](/guide/special-effects#custom-camera-effects). " +
        ...
        [Doc("Names of the components to enable or disable. The components should be attached to the same game object as the camera. " +
             "This can be used to explicitly enable or disable [custom post-processing effects](/guide/special-effects#custom-camera-effects). " +
        ...
        protected const string SelectAllSymbol = "*";
        protected virtual List<MonoBehaviour> ComponentsCache { get; } = new();

        public override Awaitable Execute (ExecutionContext ctx)
        {
            return WaitOrForget(Modify, Wait, ctx);
        }

        protected virtual async Awaitable Modify (ExecutionContext ctx)
        {
            var cam = Engine.GetServiceOrErr<ICameraManager>();

            var duration = Assigned(Duration) ? Duration.Value : cam.Configuration.DefaultDuration;
            var easingType = cam.Configuration.DefaultEasing;
            ...
            var tween = new Tween(duration, easingType, complete: !GetAssignedOrDefault(Lazy, false));
            ...
            if (Assigned(Offset)) tasks.Add(cam.ChangeOffset(ArrayUtils.ToVector3(Offset, Vector3.zero), tween, ctx.Token));

            if (Assigned(Rotation))
                tasks.Add(cam.ChangeRotation(Quaternion.Euler(
                    Rotation.ElementAtOrDefault(0) ?? cam.Rotation.eulerAngles.x,
                    Rotation.ElementAtOrDefault(1) ?? cam.Rotation.eulerAngles.y,
                    Rotation.ElementAtOrDefault(2) ?? cam.Rotation.eulerAngles.z), tween, ctx.Token));
            else if (Assigned(Roll))
                tasks.Add(cam.ChangeRotation(Quaternion.Euler(
                    cam.Rotation.eulerAngles.x,
                    cam.Rotation.eulerAngles.y,
                    Roll), tween, ctx.Token));

            if (Assigned(Zoom))
            {
                var zoom = Zoom.Value;
                if (!zoom.IsWithin(0, 1)) Warn($"Camera zoom should be in 0.0 to 1.0 range, while the assigned value is {zoom}.");
                tasks.Add(cam.ChangeZoom(zoom, tween, ctx.Token));
            }
            ...
            {
                cam.Camera.gameObject.GetComponents(ComponentsCache);
