...
namespace Naninovel
{
    ...
    {
        /// <summary>
        /// Occurs when loaded resource was reloaded due to locale change.
        /// </summary>
        public event Action<Resource<TResource>> OnLocalized;
        ...
        /// <param name="fallbackToSource">Whether to fall back to the source versions of the resources when localized versions are not available.</param>
        public LocalizableResourceLoader (List<IResourceProvider> providers, IHoldersTracker holdersTracker,
            ILocalizationManager l10n, string sourcePrefix = null, bool fallbackToSource = true)
            : base(providers, holdersTracker, sourcePrefix)
        {
            L10n = l10n;
        ...
        protected async Awaitable HandleLocaleChanged (LocaleChangedArgs _)
        {
            InitializeProvisionSources();
            ...
            async Awaitable ReloadIfLocalized (LoadedResource resource)
            {
                if (!resource.Valid || !IsLocalized(resource)) return;
            ...
            bool IsLocalized (LoadedResource resource)
            {
                foreach (var source in ProvisionSources)
                {
                    ...
                    if (source.Provider.Exists(fullPath)) return true;
                }
                return false;
            }
        }
