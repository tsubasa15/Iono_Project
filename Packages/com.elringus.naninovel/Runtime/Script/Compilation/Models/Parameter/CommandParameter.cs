...
namespace Naninovel
{
    ...
    public static class CommandParameter
    {
        ...
        public static IReadOnlyList<ParameterInfo> Extract (Command command)
        {
            ...
            foreach (var field in fields)
            {
                ...
                var id = field.Name;
                var alias = field.GetCustomAttribute<Command.AliasAttribute>()?.Alias;
        ...
        public static T Ref<T> (T param) where T : ICommandParameter, new()
        {
            ...
            for (int i = 0; i < from.Count; i++)
                to[i] = from[i].Kind == ParameterValuePartKind.IdentifiedText && !from[i].Id.StartsWithOrdinal(ScriptTextIdentifier.RefPrefix)
    ...
    public abstract class CommandParameter<TValue> : Nullable<TValue>, ICommandParameter
    {
        ...
        protected virtual TValue EvaluateDynamicValue ()
        {
            ...
            for (int i = 0; i < raw.Parts.Count; i++)
                if (raw.Parts[i].Kind == ParameterValuePartKind.Expression)
                    parts[i] = RawValuePart.FromPlainText(
                        ExpressionEvaluator.Evaluate<string>(raw.Parts[i].Expression,
                            new() { OnError = LogEvaluationError, Parameter = new(this, i) }));
        ...
        protected virtual string InterpolatePlainText (IReadOnlyList<RawValuePart> parts)
        {
            ...
            foreach (var part in parts)
                if (part.Kind != ParameterValuePartKind.PlainText)
                    Engine.Warn($"Unsupported parameter value part. Make sure you're not assigning text ID to an unsupported parameter or otherwise using '{Compiler.Symbols.TextIdOpen}' w/o escaping.", spot);
        ...
        {
            errors = null;
            if (boolText.EqualsIgnoreCase(Compiler.Symbols.True)) return true;
            if (boolText.EqualsIgnoreCase(Compiler.Symbols.False)) return false;
            errors = $"Failed to parse '{boolText}' string into boolean. Supported values: {Compiler.Symbols.True} (positive) and {Compiler.Symbols.False} (negative).";
            return false;
        }
