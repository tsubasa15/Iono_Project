...
namespace Naninovel.UI
{
    ...
    public class ImageCrossfader : IDisposable
    {
        ...
        public async Awaitable Crossfade (Texture texture, float duration)
        {
            if (tweener.Running)
            {
            ...
            var tw = new FloatTween(transitionProgress, 1, new(duration), value => transitionProgress = value);
            await tweener.Run(tw, target: material);
