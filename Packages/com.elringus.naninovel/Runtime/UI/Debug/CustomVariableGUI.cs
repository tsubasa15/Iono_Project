...
namespace Naninovel.UI
{
    public class CustomVariableGUI : MonoBehaviour
    {
        ...
        {
            if (!instance) instance = Engine.CreateObject<CustomVariableGUI>(new() { Name = nameof(CustomVariableGUI) });
            show = !show;
        ...
        private void OnEnable ()
        {
            ...
            state.OnGameLoadFinished += HandleGameLoadFinished;
            state.OnResetFinished += UpdateRecords;
            state.OnRollbackFinished += UpdateRecords;
        }
        ...
        private void DrawWindow (int windowId)
        {
            ...
            foreach (var record in records)
            {
                if (!string.IsNullOrEmpty(search) &&
                    !record.Key.StartsWith(search, StringComparison.OrdinalIgnoreCase)) continue;
        ...
        {
            if (editedValue.StartsWithOrdinal("\"") && editedValue.EndsWithOrdinal("\""))
                vars.SetVariableValue(name, new(editedValue.Substring(1, editedValue.Length - 2)));
            else if (bool.TryParse(editedValue, out var boo))
                vars.SetVariableValue(name, new(boo));
            else if (ParseUtils.TryInvariantFloat(editedValue, out var num))
                vars.SetVariableValue(name, new(num));
        }
