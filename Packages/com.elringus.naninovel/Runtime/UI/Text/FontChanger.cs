...
{
    [AddComponentMenu("Naninovel/ UI/Font Changer")]
    public class FontChanger : MonoBehaviour
    {
        ...
        public virtual void ApplySelectedFont ()
        {
            if (IsFontNameChangePending())
                HandleFontNameChanged(uis.FontName);
            if (IsFontSizeChangePending())
                HandleFontSizeChanged(uis.FontSize);
        }
        ...
        {
            uis.OnFontNameChanged += HandleFontNameChanged;
            uis.OnFontSizeChanged += HandleFontSizeChanged;
            ApplySelectedFont();
        }

        protected virtual void OnDisable ()
        {
            if (uis is null) return;
            uis.OnFontNameChanged -= HandleFontNameChanged;
            uis.OnFontSizeChanged -= HandleFontSizeChanged;
        }
        ...
        {
            var font = string.IsNullOrWhiteSpace(fontName) ? null : uis.GetFontAsset(fontName);
        ...
        protected virtual bool IsFontNameChangePending ()
        {
            if (string.IsNullOrWhiteSpace(lastAppliedFontName))
                return !string.IsNullOrWhiteSpace(uis.FontName);
            return lastAppliedFontName != uis.FontName;
        }

        protected virtual bool IsFontSizeChangePending ()
        {
            return lastAppliedFontSize != uis.FontSize;
        }
    }
}
