    ...
    public class GameSettingsPreviewPrinter : ScriptableUIBehaviour
    {
        ...
        protected override void Awake ()
        {
            ...
            if (RevealableText is null)
                throw Engine.Fail($"Field `{nameof(revealableText)}` on `{nameof(GameSettingsPreviewPrinter)}` " +
                                  $"component is either not assigned or doesn't implement `{nameof(IRevealableText)}` interface.");
        ...
        protected virtual async Awaitable RevealTextOverTime (float revealDelay, CancellationToken token)
        {
