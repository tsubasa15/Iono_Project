...
namespace Naninovel.UI
{
    ...
    public class TipsPanel : CustomUI, ITipsUI
    {
        ...
        [Header("Events")]
        [SerializeField] private StringUnityEvent onItemSelected;
        ...
        {
            await FillListItems();
            BindInput(Inputs.Navigate, HandleNavigationInput);
            BindInput(Inputs.Cancel, Hide, new() { OnEnd = true });
        }

        public virtual async void SelectTipRecord (string tipId)
        {
        ...
        protected virtual async Awaitable FillListItems ()
        {
        ...
        protected virtual async Awaitable FillListItem (ManagedTextRecord record)
        {
        ...
        protected virtual void SelectItem (TipsListItem item)
        {
            ...
            foreach (var listItem in listItems)
                listItem.SetSelected(listItem.UnlockableId.EqualsOrdinal(item.UnlockableId));
            ...
            onItemSelected?.Invoke(item.UnlockableId);
        }

        protected virtual void HandleUnlockableItemUpdated (UnlockableItemUpdatedArgs args)
        {
            if (!args.Id.StartsWithOrdinal(UnlockableIdPrefix)) return;

            var unlockedItem = listItems.Find(i => i.UnlockableId.EqualsOrdinal(args.Id));
        ...
        protected virtual async Awaitable HandleLocaleChanged (LocaleChangedArgs _)
        {
            ClearSelection();
            ClearListItems();
            await FillListItems();
        ...
        public override async Awaitable ChangeVisibility (bool visible, float? duration = null, AsyncToken token = default)
        {
            if (!visible)
        ...
        protected virtual void HandleNavigationInput (Vector2 force)
        {
            if (!Visible) return;
            if (force.y <= -1f) SelectNextUnlockedItem();
            if (force.y >= 1f) SelectPreviousUnlockedItem();
        }
