...
namespace Naninovel
{
    ...
    public class StateRollbackStack : IEnumerable<GameStateMap>
    {
        ...
        public GameStateMap PopExhaustive (Predicate<GameStateMap> predicate)
        {
            var lastMatchingNode = default(LinkedListNode<GameStateMap>);
            for (var node = rollbackList.First; node != null; node = node.Next)
                if (predicate(node.Value))
                    lastMatchingNode = node;
            if (lastMatchingNode is null) return null;
            while (rollbackList.First != lastMatchingNode)
                Pop();
            return Pop();
        }
