...
namespace Naninovel
{
    ...
    public class SceneBackground : MonoBehaviourActor<BackgroundMetadata>, IBackgroundActor, Blur.IBlurable
    {
        ...
        protected virtual async Awaitable<SceneData> GetOrLoadScene (string appearance, AsyncToken token)
        {
            // Using lock as it can be invoked multiple times
            // without awaiting when preloading script resources.
            using var _ = await loadSemaphore.Wait(token.CancellationToken);
        ...
        protected virtual async Awaitable<Scene> LoadScene (string appearance, AsyncToken token)
        {
