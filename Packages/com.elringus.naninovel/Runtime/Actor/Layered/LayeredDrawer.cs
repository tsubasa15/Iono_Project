    ...
    public class LayeredDrawer : IDisposable
    {
        ...
        public virtual RenderTexture DrawLayers (float pixelsPerUnit, RenderTexture renderTexture = default)
        {
            ...
            if (layers is null || layers.Count == 0)
                throw Engine.Fail($"Can't render layered actor '{transform.name}': layers data is empty. Make sure the actor prefab contains child objects with at least one renderer.");
            ...
            var drawPosition = (Vector2)transform.TransformPoint((Vector2)transform.localPosition + canvasRect.position) * pixelsPerUnit;
            var orthoMin = Vector2.Scale(-drawDimensions / 2f, parent.localScale) + drawPosition;
            var orthoMax = Vector2.Scale(drawDimensions / 2f, parent.localScale) + drawPosition;
        ...
        protected virtual Rect EvaluateCanvasRect ()
        {
            ...
            for (var i = 0; i < layers.Count; i++)
                if (layers[i] is LayeredRendererLayer layer)
                    bounds.Encapsulate(layer.Renderer.bounds);
            return new(transform.InverseTransformPoint(bounds.center), bounds.size);
        }
        ...
        protected virtual void BuildCameraLayers ()
        {
            ...
            if (cullingToUsed.Count == 0)
                for (int i = 8; i <= 31; i++)
                    if (LayerMask.LayerToName(i).StartsWithOrdinal("Naninovel"))
        ...
        protected virtual void HoldCameraLayer ()
        {
            ...
            if (!cullingLayer.HasValue)
                throw Engine.Fail($"Failed to to render '{transform.GetComponentInParent<LayeredActorBehaviour>().name}' layered actor in camera mode: " +
