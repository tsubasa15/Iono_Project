...
namespace Naninovel
{
    ...
    public class LayeredRendererLayer : ILayeredRendererLayer
    {
        ...
        private static readonly int urpMainTexId = Shader.PropertyToID("_MainTex");
        private static readonly int urpSpriteColorId = Shader.PropertyToID("unity_SpriteColor");
        private static readonly int urpSpritePropsId = Shader.PropertyToID("unity_SpriteProps");
        ...
        public void Dispose ()
        {
            if (Mesh && Mesh.hideFlags.HasFlag(HideFlags.HideAndDontSave) && Mesh.name.EqualsOrdinal("Generated Mesh"))
        ...
        public MaterialPropertyBlock GetPropertyBlock (MaterialPropertyBlock block)
        {
            ...
            {
                #if URP_AVAILABLE
                if (spriteRenderer.sprite) block.SetTexture(urpMainTexId, spriteRenderer.sprite.texture);
                block.SetColor(urpSpriteColorId, spriteRenderer.color);
                block.SetVector(urpSpritePropsId, new(1, 1, 0, 0));
                #else
