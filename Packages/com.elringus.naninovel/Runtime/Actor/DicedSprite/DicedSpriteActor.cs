...
namespace Naninovel
{
        ...
        where TMeta : OrthoActorMetadata
    {
        ...
        public override async Awaitable ChangeAppearance (string appearance, Tween tween, Transition? transition = default, AsyncToken token = default)
        {
            base.Appearance = appearance;
            var atlas = await GetOrLoadAtlas(token);
            if (base.Appearance != appearance) return;
        ...
        protected virtual async Awaitable<DicedSpriteAtlas> GetOrLoadAtlas (AsyncToken token)
        {
