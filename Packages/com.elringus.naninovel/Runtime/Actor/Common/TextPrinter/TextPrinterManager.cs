...
namespace Naninovel
{
        ...
        TextPrinterMetadata, TextPrintersConfiguration>, IStatefulService<SettingsStateMap>, ITextPrinterManager
    {
        ...
        public TextPrinterManager (TextPrintersConfiguration cfg, CameraConfiguration cameraCfg, IScriptPlayer player)
            : base(cfg, cameraCfg)
        {
            Player = player;
        }
        ...
        public virtual async Awaitable Print (string printerId, PrintedMessage message,
            bool append = false, float speed = 1, AsyncToken token = default)
        {
            ...
            var delay = (meta.RevealInstantly || Player.Skipping) ? 0
