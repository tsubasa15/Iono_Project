...
namespace Naninovel
{
    ...
    public class TextPrintersConfiguration : OrthoActorManagerConfiguration<TextPrinterMetadata>
    {
        ...
        [Range(0f, 1f), Tooltip("When above zero, each print command will wait for the specified time (in seconds, unscaled) while the skip playback mode (fast-forward) is enabled. Use to slow down the playback while skipping.")]
        public float SkipPrintDelay;
