...
namespace Naninovel
{
    ...
    public abstract class ActorManagerConfiguration : Configuration
    {
        ...
        [ContextMenu("Remove Invalid Metadata")]
        public void RemoveInvalidMetadata ()
        {
            // Remove metadata entries with duplicate GUIDs.
            using var _ = ListPool<string>.Rent(out var ids);
            using var __ = SetPool<string>.Rent(out var guids);
            MetadataMap.CollectIds(ids);
            foreach (var id in ids)
                if (MetadataMap.GetMetadata(id) is not { } meta || !guids.Add(meta.Guid))
                {
                    MetadataMap.RemoveRecord(id);
                    Debug.LogWarning($"Removed '{id}' duplicate metadata entry.");
                }
            #if UNITY_EDITOR
