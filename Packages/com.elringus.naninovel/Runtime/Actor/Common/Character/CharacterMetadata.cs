...
namespace Naninovel
{
    ...
    public class CharacterMetadata : OrthoActorMetadata
    {
        ...
        {
            public Pose () { }
            public Pose (string name, CharacterState actorState, params string[] overriddenProperties)
                : base(name, actorState, overriddenProperties) { }
        }
        ...
        [Tooltip("Virtual reference points that can be used by various systems to anchor to a specific location inside the character.")]
        public List<Anchor> Anchors = new();

        public CharacterMetadata ()
        {
            Implementation = typeof(PlaceholderCharacter).AssemblyQualifiedName;
