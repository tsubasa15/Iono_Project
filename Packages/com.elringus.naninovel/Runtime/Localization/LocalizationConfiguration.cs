...
namespace Naninovel
{
    ...
    public class LocalizationConfiguration : Configuration
    {
        ...
        [Tooltip("Whether to make the source locale available to the end-users (players), ie included to the language selection.\n\nDisabling this option can be useful in case you'd like to share the source localizable text with a third-party (eg, for proofreading), but don't want to share the scenario scripts. In which case, disable this option and add a dedicated locale for the source material, which can then be exported to the localization documents or spreadsheets.")]
        public bool ExposeSourceLocale = true;
