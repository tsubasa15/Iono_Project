...
namespace Naninovel
{
    ...
    public class LocalizationEjector
    {
        ...
        {
            return Environment.GetCommandLineArgs().Any(a => a.StartsWithOrdinal(ejectArg));
        }

        public async Awaitable EjectScripts (string outDir)
        {
        ...
        public async Awaitable EjectText (string outDir)
        {
        ...
        private string ResolveEjectionLocale ()
        {
            var locale = Environment.GetCommandLineArgs()
                .First(a => a.StartsWithOrdinal(ejectArg)).GetAfter(ejectArg + "-")?.TrimFull();
