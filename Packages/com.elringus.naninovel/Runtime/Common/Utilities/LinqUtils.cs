...
namespace Naninovel
{
    public static class LinqUtils
    {
        ...
        /// <summary>
        /// Clears the source collection and adds elements from the specified enumerable.
        /// </summary>
        ...
        /// <summary>
        /// Rents a pooled list and populates it with the elements from the source enumerable.
        /// </summary>
        /// <param name="source">Source collection from which to copy elements.</param>
        /// <param name="rented">The rented list populated with elements from the source.</param>
        public static IDisposable RentWith<T> (this IEnumerable<T> source, out List<T> rented)
        {
            var rent = ListPool<T>.Rent(out rented);
            rented.AddRange(source);
            return rent;
        }
            ...
            void Visit (T item)
            {
                ...
                if (alreadyVisited)
                {
                    if (inProcess && warnCyclic)
                        Engine.Warn($"Cyclic dependency found while performing topological ordering of {typeof(T).Name}.");
                }
                else
                {
                    visited[item] = true;
