...
namespace Naninovel
{
    ...
    public readonly struct AsyncToken : IEquatable<AsyncToken>
    {
        ...
        /// A canceled token.
        /// </summary>
        public static readonly AsyncToken CanceledToken = new(new(true));
        /// <summary>
        /// A completed token.
        /// </summary>
        public static readonly AsyncToken CompletedToken = new(default, new(true));
        ...
        /// <summary>
        /// Creates shared cancellation token of the combined cancel and complete tokens.
        /// Don't forget to dispose the returned object to prevent memory leaks.
        /// </summary>
        public IDisposable GetShared (out CancellationToken token)
        {
            if (!CancellationToken.CanBeCanceled && !CompletionToken.CanBeCanceled)
            {
                token = CancellationToken.None;
                return new DeferNoop();
            }
            var cts = CancellationTokenSource.CreateLinkedTokenSource(CancellationToken, CompletionToken);
            token = cts.Token;
            return cts;
        }
