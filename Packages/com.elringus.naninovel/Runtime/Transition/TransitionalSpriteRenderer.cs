...
namespace Naninovel
{
    ...
    public class TransitionalSpriteRenderer : TransitionalRenderer
    {
        ...
        #if URP_AVAILABLE
        private const string depthShaderName = "Hidden/Naninovel/DepthMaskURP";
        #else
        private const string depthShaderName = "Hidden/Naninovel/DepthMaskBiRP";
        #endif
        ...
        public virtual void Initialize (Vector2 pivot, float pixelsPerUnit, bool premultipliedAlpha, AspectMatchMode matchMode,
            float matchRatio, FilterMode filterMode, bool depthPassEnabled, Material customMaterial = default, Material customSpriteMaterial = default)
        {
            ...
            if (depthPassEnabled) meshRenderer.materials = new[] { SpriteMaterial, DepthMaterial };
            else meshRenderer.material = SpriteMaterial;
        }

        protected virtual void Update ()
        {
            var render = ShouldRender();
            if (render && meshRenderer.forceRenderingOff) meshRenderer.forceRenderingOff = false;
            if (!render && !meshRenderer.forceRenderingOff) meshRenderer.forceRenderingOff = true;
            if (!render) return;
