...
namespace Naninovel.E2E
{
    ...
    public static class Extensions
    {
        ...
        public static E2E WithFastForward (this E2E e2e) => e2e
            .With(() => Service<IScriptPlayer>().OnAwaitInput += _ => Input(Inputs.Continue).Pulse())
        ...
        public static ISequence Play (this ISequence seq, ISequence sequence) =>
            sequence != null ? seq.Enqueue(() => sequence.ToAwaitable()) : seq;
        ...
        public static ISequence Once (this ISequence seq, Func<bool> condition, float timeout = 10,
            ...
            try { return Async.Until(condition, cts.Token).Then(cts.Dispose); }
            catch (OperationCanceledException)
            {
                Fail("Timeout.", file, line);
        ...
        /// When 'continue' condition is specified, will check it after executing 'do', and, when met,
        /// reset the timeout and repeat.
        /// </summary>
        public static ISequence On (this ISequence seq, Func<bool> condition, ISequence @do,
            Func<bool> @continue = null, float timeout = 10, [CallerFilePath] string file = "",
            [CallerLineNumber] int line = 0) => seq.Enqueue(async () => {
            do
            {
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(timeout));
                try { await Async.Until(condition, cts.Token); }
                catch (OperationCanceledException) { Fail("Timeout.", file, line); }
                if (@do != null) await @do.ToAwaitable();
            }
        ...
        public static ISequence Wait (this ISequence seq, float seconds) =>
            seq.Enqueue(() => Async.Delay(TimeSpan.FromSeconds(seconds)));
        ...
        public static ISequence Choose (this ISequence seq, string id = null,
            [CallerFilePath] string file = "", [CallerLineNumber] int line = 0) => seq.Enqueue(() => {
            var filter = new Predicate<Choice>(c => c.Summary.Parts.Any(p => id is null || p.Id == id));
            var handler = Choices.FindActor(a => a.Visible && a.AnyChoice(filter));
            if (handler == null) Fail($"Failed to choose '{id}': no choice handlers with the specified summary ID are visible.", file, line);
            handler.HandleChoice(handler.FindChoice(filter)!.Value.Id);
        });
    }
}
