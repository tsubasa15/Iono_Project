...
namespace Naninovel.E2E
{
    ...
    #endif
    {
        ...
        private readonly PlayedScriptRegister played = new();
        ...
        {
            if (Enabled) ReportCoverage(played.GetPlayed()).Forget();
        }
        #endif
        ...
        private void RegisterPlayed ()
        {
            ...
            var state = global.GetState<ScriptPlayer.GlobalState>()?.Played;
            if (state == null) return;
            foreach (var kv in state.GetPlayed())
            foreach (var range in kv.Value)
                for (int i = range.StartIndex; i <= range.EndIndex; i++)
                    played.RegisterPlayedIndex(kv.Key, i);
        }

        private static async Awaitable ReportCoverage (IReadOnlyDictionary<string, IReadOnlyList<IntRange>> played)
        {
